...
class ScheduleService(
    ...
    suspend fun getScheduleByCourses(
        ...
        return repo.getScheduleByCourses(
            delivery,
            courses,
            campus,
            term,
    ...
    suspend fun getFillerByAttributes(
        ...
        return repo.getFillerByAttributes(
            delivery,
            attributes,
            courses,
            campus,
...
private suspend fun validateScheduleFields(
    delivery:List<String>?=null,
    ...
    if ((!preferredStart.isNullOrEmpty()&&preferredEnd.isNullOrEmpty())
        ||(!preferredEnd.isNullOrEmpty() && preferredStart.isNullOrEmpty())) throw IllegalArgumentException("Preferred start and end fields must be specified together")
    if ((preferredStart!=null && preferredEnd!=null)&&
        repo.toMinutes(preferredStart)>=repo.toMinutes(preferredEnd)) throw IllegalArgumentException("Preferred start must be before preferred end")
}
