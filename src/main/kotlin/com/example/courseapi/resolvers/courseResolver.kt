...
class CourseResolver(private val service: CourseService, private val logger: Logger = LoggerFactory.getLogger(CourseResolver::class.java), ) {
    @QueryMapping
    suspend fun getCourses(@Argument subject: List<String>?, @Argument courseNum: Int?, @Argument campus: List<String>?, @Argument attributes: List<String>?, @Argument delivery: List<String>?, @Argument term: String?): CourseResult {
        return try{ CourseResult.Success(service.getCourses(subject, courseNum, campus, attributes, delivery, term)) }
